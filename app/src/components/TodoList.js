...
function TodoList() {
    ...
    let [todos, setTodosState] = useState(() => {
    ...
    let [todoIdNext, setTodoIdNextState] = useState(() => {
    ...
    let todosFiltered = todos
        ...
        .filter(item => {
            ...
            Object.entries(filters).forEach((filter) => {
                if (item.data[filter[0]] !== filter[1]) {
                    includeItem = false
                }
            })
            return includeItem
    ...
    return (
        <>
            ...
            <p className="me-2">
                {todosFilteredCompleted.length} item(s) remaining
            </p>
            <TodoListActions
                todos={todos}
                setTodos={setTodos}
